  ...
  async findAllBase(
    ...
    if (
      ...
      for (const term of SEARCH_TERMS) {
        // Check if the search term is a nested relation
        if (term?.includes('.')) {
          const fields = term.split('.'); // Split by '.' for nested relations
          let relationObject: any = { [fields.pop()!]: ILike(`%${searchTerm}%`) }; // Initialize with the last field

          // Build nested relation structure in reverse
          while (fields.length > 0) {
            relationObject = { [fields.pop()!]: relationObject };
          }
          ...
          // Handle non-nested search terms
          where.push({
            ...queryOptions,
